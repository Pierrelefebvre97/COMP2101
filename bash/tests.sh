...
[ -x /tmp ] && echo "/tmp is accesable" || echo "/tmp is not accesable"
...
multipleTest=$(($secondNumber % $firstNumber))
...
[ $firstNumberMod -eq 1 ] && echo "the first number is odd" || echo "the first number is even"
[ $secondNumberMod -eq 1  ] && echo "the second number is odd" || echo "the second number is even"
[ $multipleTest -eq 0  ] && echo "the second number is a multiple of the first" || echo "the second number is not a multiple of the first"
...
[ -v USER ] && echo "$USER is what is inside the variable" || echo "the variable is empty"
