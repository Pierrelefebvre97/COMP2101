...
while [ -z $userguess ]; do
  read -p "Please type a number between 1 and 10" userguess
done
while (($userguess > 10 )); do
  read -p "your number cant be higher than 10 choose again" userguess
done
while  (($userguess < 1)) ; do
  read -p "your number cant be lower than 1 choose again" userguess
